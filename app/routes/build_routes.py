...
PATCH: Added _merge_sections() to prevent the LLM from silently
...
def _merge_sections(original: dict, rebuilt: dict) -> dict:
    ...
    This prevents sections like 'languages' and 'projects' from being
    silently dropped when the LLM forgets to include them.
    """
    merged = {}
    ...
    for key in SECTION_ORDER:
        ...
        if rebuilt_val:
            merged[key] = rebuilt_val       # prefer AI-rewritten version
        elif original_val:
            merged[key] = original_val      # fall back to original
        # both empty → omit

    # Append any extra keys not in SECTION_ORDER
    extra_keys = sorted(
...
def rebuild_cv():
    ...
    # Restore any sections the LLM omitted
    ...
    if error:
        current_app.logger.error(f"Re-scoring rebuilt CV failed for analysis {analysis_id}: {error}")
...
def check_rebuild_exists(analysis_id):
    ...
    if generated:
        return jsonify({"exists": True, "generated_id": generated.id,
                        "rebuilt_score": generated.rebuilt_score,
                        "score_delta": generated.score_delta}), 200
